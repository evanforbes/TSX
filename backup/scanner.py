...
from config import (
    ...
    GOLD_STANDARD_MIN_SIGNALS, SILVER_STANDARD_MIN_SIGNALS
)
...
def classify_signal_tier(result: dict) -> dict:
    """
    Classify stock into Gold (3+), Silver (2+), or regular tier

    Returns dict with:
        - tier_buy: 'GOLD', 'SILVER', or None
        - tier_sell: 'GOLD', 'SILVER', or None
    ...
    # Determine buy tier
    tier_buy = None
    if len(buy_signals) >= GOLD_STANDARD_MIN_SIGNALS:
        tier_buy = 'GOLD'
    ...
    # Determine sell tier
    tier_sell = None
    if len(sell_signals) >= GOLD_STANDARD_MIN_SIGNALS:
        tier_sell = 'GOLD'
...
def print_results(results: list):
    ...
    for r in results:
        ...
        # Check for Gold/Silver tiers
        if classification['tier_buy'] == 'GOLD':
    ...
    # Print Gold Standard Buys (TOP PRIORITY)
    if gold_buys:
        print("\n" + "=" * 80)
